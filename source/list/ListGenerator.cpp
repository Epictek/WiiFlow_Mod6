...
void ListGenerator::Init(const char *settingsDir, const char *Language, const char *plgnsDataDir)
{
	...
	if(plgnsDataDir != NULL)
		pluginsDataDir = fmt("%s", plgnsDataDir);
}
...
static void AddISO(const char *GameID, const char *GameTitle, const char *GamePath, u32 GameColor, u8 Type)
{
	...
	if(GamePath != NULL) 
		strncpy(ListElement.path, GamePath, sizeof(ListElement.path) - 1);
...
static void Create_Wii_WBFS_List(wbfs_t *handle)
{
	for(u32 i = 0; i < wbfs_count_discs(handle); i++)
	{
		...
		if(ret == 0 && wii_hdr.magic == WII_MAGIC)
			AddISO((const char*)wii_hdr.id, (const char*)wii_hdr.title, NULL, 0xFFFFFF, TYPE_WII_GAME);
	}
}
...
static void Add_Wii_Game(char *FullPath)
{
	...
	if(fp)
	{
		...
		if(wii_hdr.magic == WII_MAGIC)
			AddISO((const char*)wii_hdr.id, (const char*)wii_hdr.title, FullPath, 0xFFFFFF, TYPE_WII_GAME);
		fclose(fp);
	}
}
...
void ListGenerator::CreateList(u32 Flow, u32 Device, const string& Path, const vector<string>& FileTypes, const string& DBName, bool UpdateCache)
{
	Clear();
	if(!DBName.empty())
	{
	...
	if(Flow == COVERFLOW_WII)
	{
		if(DeviceHandle.GetFSType(Device) == PART_FS_WBFS)
			...
			GetFiles(Path.c_str(), FileTypes, Add_Wii_Game, false, 30); // added 30 subfolders
	}
	...
	else if(DeviceHandle.GetFSType(Device) != PART_FS_WBFS)
	{
		if(Flow == COVERFLOW_GAMECUBE)
			GetFiles(Path.c_str(), FileTypes, Add_GameCube_Game, true, 30); // true means to look for a folder (/root) + added 30 subfolders
...
dirent *pent = NULL;
DIR *pdir = NULL;
void GetFiles(const char *Path, const vector<string>& FileTypes, FileAdder AddFile, bool CompareFolders, u32 max_depth, u32 depth)
{
	...
	while((pent = readdir(pdir)) != NULL)
	{
		...
		if(pent->d_type == DT_DIR)
		{
			...
			// else if(depth < max_depth && strcmp(pent->d_name, "folder_to_skip") != 0)
...
void ListGenerator::createSFList(u8 maxBtns, Config &m_sourceMenuCfg, const string& sourceDir)
{
	...
	for(u8 i = 0; i <= maxBtns; i++)
	{
		...
		strncpy(btn_selected, fmt("BUTTON_%i", i), 15);
		...
		strncpy(SourceTitle, m_sourceMenuCfg.getString(btn_selected, "title", "").c_str(), 63); // removed "title_i"
