...
{
	if(wbfs_part_fs) 
		return WBFS_Ext_RemoveGame(discid, path);
...
s32 WBFS_GameSize(u8 *discid, char *path, f32 *size)
{
	...
	u32 sectors = wbfs_disc_sector_used(disc); // change after modifying install game feature
...
{
	if(wbfs_part_fs) 
		return WBFS_Ext_DVD_Size(comp_size, real_size);
...
{
	if(wbfs_part_fs) 
		return WBFS_Ext_DiskSpace(used, free);
