...
// from USB Loader GX with minor modifications
...
wbfs_t *wbfs_open_hd(rw_sector_callback_t read_hdsector, rw_sector_callback_t write_hdsector, void *callback_data, int hd_sector_size, int num_hd_sector __attribute( ( unused ) ), int reset) // not used
{
...
wbfs_t *wbfs_open_partition(rw_sector_callback_t read_hdsector, rw_sector_callback_t write_hdsector, void *callback_data, int hd_sector_size, int num_hd_sector, u32 part_lba, int reset)
{
	...
	wbfs_head_t *head = wbfs_ioalloc(hd_sector_size ? hd_sector_size : 512); // instead of wbfs_malloc()
...
// int install_abort_signal = 0; // not used in Wiiflow
// s32 wbfs_add_disc(wbfs_t*p, read_wiidisc_callback_t read_src_wii_disc, void *callback_data, progress_callback_t spinner, partition_selector_t sel, int copy_1_1)
s32 wbfs_add_disc(wbfs_t*p, read_wiidisc_callback_t read_src_wii_disc, void *callback_data, progress_callback_t spinner, void *spinner_data, partition_selector_t sel, int copy_1_1)
{
	int i, discn, ret;
	u32 tot, cur;
	...
	if(copy_1_1)
	{
		...
		if((last_used + 1) > (p->n_wbfs_sec_per_disc / 2)) // dual layer
			num_wbfs_sect_to_copy = p->n_wbfs_sec_per_disc;
		else // single layer
...
/*********************************************************************************************************/
/** Functions in libwbfs.c from USB Loader GX not used in Wiiflow **/
