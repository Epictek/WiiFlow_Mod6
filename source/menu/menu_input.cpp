...
bool using_exp[WPAD_MAX_WIIMOTES] = {false, false, false, false};
...
void CMenu::ScanInput(bool enlargeButtons)
{
	/*
	for(int chan = WPAD_MAX_WIIMOTES-1; chan >= 0; chan--)
	{
	...
	*/
	...
	m_show_zone_header = false; // menu home
	m_show_zone_footer = false; // menu home
	
	m_show_zone_prev = false; // menu main
	m_show_zone_next = false; // menu main
	
	for(u8 i = 0; i < 7; ++i)
		m_show_zone_info[i] = false;
	for(u8 i = 0; i < 6; ++i)
		m_show_zone_gameinfo[i] = false;
	...
	for(int chan = WPAD_MAX_WIIMOTES-1; chan >= 0; chan--)
	{
		...
		if(WPadIR_Valid(chan)) // wiimote IR
		{
			...
			m_btnMgr.mouse(chan, wd[chan]->ir.x - m_cursor[chan].width() / 2, wd[chan]->ir.y - m_cursor[chan].height() / 2, enlargeButtons);
		}
		else if(m_show_pointer[chan]) // left stick
		{
			m_cursor[chan].draw(stickPointer_x[chan], stickPointer_y[chan], 0);
			m_btnMgr.mouse(chan, stickPointer_x[chan] - m_cursor[chan].width() / 2, stickPointer_y[chan] - m_cursor[chan].height() / 2, enlargeButtons);
		}
	}
	/* Handles display zones */
	ShowHeaderZone(); // menu home / exitto
	ShowFooterZone(); // menu home / exitto
	
	ShowPrevZone(); // menu main
	ShowNextZone(); // menu main
	
	ShowMainInfoZone(); // menu main
	ShowGameInfoZone(); // menu game
}
...
bool CMenu::wBtn_Pressed(int btn, u8 ext)
{
	for(int chan = WPAD_MAX_WIIMOTES-1; chan >= 0; chan--)
	{
		...
		{
			activeChan = chan; // indicates last WPAD used for coverflow and gui select functions
			return true;
		}
	}
	return false;
...
also in menu_explorer to highlight files and folder quickly
btn A repeat is used in adjust coverflow and sound volume adjusting */
...
/****************************************************************/
/** LEFT and RIGHT are reversed on Wiimote only (no expansion) **/
/****************************************************************/
	else if(btn == WBTN_RIGHT_REV)
	{
		if(WBTN_RIGHT_REV_HELD)
		{
			if(m_wpadRightDelay == 0 || m_wpadRightDelay >= g_repeatDelay)
				b = true;
			if(m_wpadRightDelay < g_repeatDelay) 
	...
	else if(btn == WBTN_LEFT_REV)
	{
		if(WBTN_LEFT_REV_HELD)
		{
			if(m_wpadLeftDelay == 0 || m_wpadLeftDelay >= g_repeatDelay)
				b = true;
			if(m_wpadLeftDelay < g_repeatDelay) 
...
/* Home menu header and footer */
void CMenu::ShowHeaderZone()
{
	ShowZone(m_homeButtonsHeader, m_show_zone_header);
}

void CMenu::ShowFooterZone()
{
	ShowZone(m_homeButtonsFooter, m_show_zone_footer);
}
...
/* Icon info in main and game menu */
void CMenu::ShowMainInfoZone()
{
	for(u8 i = 0; i < 7; ++i)
		ShowZone(m_mainInfo[i], m_show_zone_info[i]);
}

void CMenu::ShowGameInfoZone()
{
	for(u8 i = 0; i < 6; ++i)
		ShowZone(m_gameInfo[i], m_show_zone_gameinfo[i]);
}
