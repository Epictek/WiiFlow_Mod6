...
void CMenu::_showConfigMisc(bool instant)
{
	...
	m_btnMgr.setText(m_configLbl[1], _t("cfg808", L"Full-Scene Anti-Aliasing"));
	m_btnMgr.setText(m_configLblVal[1], wfmt(L"%i", m_cfg.getInt(general_domain, "max_fsaa", 3)));
	//! Cover buffer
	m_btnMgr.setText(m_configLbl[2], _t("cfg809", L"Cover buffer"));
	m_btnMgr.setText(m_configLblVal[2], wfmt(L"%i", m_cfg.getInt(general_domain, "cover_buffer", 20)));
	//! Fanart delay
	m_btnMgr.setText(m_configLbl[3], _t("cfg810", L"Fanart animation delay"));
	m_btnMgr.setText(m_configLblVal[3], wfmt(L"%i", m_cfg.getInt(general_domain, "fanart_delay", 8)));
	//! Fanart
	m_btnMgr.setText(m_configLbl[4], _t("cfg706", L"Enable fanart"));
	m_checkboxBtn[4] = m_cfg.getBool(general_domain, "enable_fanart", 0) == 0 ? m_configChkOff[4] : m_configChkOn[4];
	//! Overwrite titles_dump.ini
	m_btnMgr.setText(m_configLbl[5], wfmt(_fmt("cfg819", L"Overwrite %s"), TITLES_DUMP_FILENAME));
	m_checkboxBtn[5] = m_cfg.getBool(general_domain, "overwrite_dump_list", 0) == 0 ? m_configChkOff[5] : m_configChkOn[5];
	//! Update Wii desktop playlog
	m_btnMgr.setText(m_configLbl[6], _t("cfg812", L"Update Wii playlog"));
	m_checkboxBtn[6] = m_cfg.getBool(general_domain, "playlog_update", 0) == 0 ? m_configChkOff[6] : m_configChkOn[6];
	...
	//! SD gecko debug
	m_btnMgr.setText(m_configLbl[7], _t("cfg788", L"Enable SD debug next boot"));
	m_checkboxBtn[7] = m_cfg.getBool("DEBUG", "sd_write_log", 0) == 0 ? m_configChkOff[7] : m_configChkOn[7];
	//! Show memory usage on screen
	m_btnMgr.setText(m_configLbl[8], _t("cfg786", L"Show memory next boot"));
	m_checkboxBtn[8] = m_cfg.getBool("DEBUG", "show_mem", 0) == 0 ? m_configChkOff[8] : m_configChkOn[8];
...
{	
	...
	while(!m_exit)
	{
		...
		else if(BTN_A_OR_2_PRESSED)
		{
			...
			else if(m_btnMgr.selected(m_configBtnP[1]) || m_btnMgr.selected(m_configBtnM[1])) // FSAA
			{
			...
			else if(m_btnMgr.selected(m_configBtnP[2]) || m_btnMgr.selected(m_configBtnM[2])) // cover buffer
			{
			...
			else if(m_btnMgr.selected(m_configBtnP[3]) || m_btnMgr.selected(m_configBtnM[3])) // fanart delay
			{
			...
			else if(m_btnMgr.selected(m_checkboxBtn[4])) // enable fanart
			{
			...
			else if(m_btnMgr.selected(m_checkboxBtn[5])) // overwrite titles_dump.ini
			{
			...
			else if(m_btnMgr.selected(m_checkboxBtn[6])) // update playlog
			{
...
#ifdef DEBUG
			else if(m_btnMgr.selected(m_checkboxBtn[7])) // enable sd debug
			{
				m_cfg.setBool("DEBUG", "sd_write_log", !m_cfg.getBool("DEBUG", "sd_write_log"));
				_showConfigMisc(true);
				m_btnMgr.setSelected(m_checkboxBtn[7]);
			}
			else if(m_btnMgr.selected(m_checkboxBtn[8])) // show mem
			{
