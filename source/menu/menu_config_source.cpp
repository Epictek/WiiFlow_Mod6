...
void CMenu::_showConfigSource(bool instant)
{
	...
	m_btnMgr.setText(m_configLbl[0], _t("cfg718", L"Source menu on start"));
	m_checkboxBtn[0] = m_cfg.getOptBool(general_domain, "source_on_start", 0) == 0 ? m_configChkOff[0] : m_configChkOn[0];
	//! Enable sourceflow
	m_btnMgr.setText(m_configLbl[1], _t("cfgsm3", L"Enable sourceflow")); // default false
	m_checkboxBtn[1] = SF_enabled ? m_configChkOn[1] : m_configChkOff[1];
	//! Sourceflow smallbox
	m_btnMgr.setText(m_configLbl[2], _t("cfgsm4", L"Sourceflow smallbox")); // default true
	m_checkboxBtn[2] = cur_smallbox ? m_configChkOn[2] : m_configChkOff[2];
	...
	//! Sort sourceflow
	m_btnMgr.setText(m_configLbl[4], _t("cfg811", L"Sort sourceflow alphabetically"));
	m_checkboxBtn[4] = m_cfg.getInt(sourceflow_domain, "sort", SORT_ALPHA) == SORT_ALPHA ? m_configChkOn[4] : m_configChkOff[4];
	//! Hide source buttons
	m_btnMgr.setText(m_configLbl[5], _t("smedit5", L"Hide source buttons"));
	//! Link source buttons to plugins
	m_btnMgr.setText(m_configLbl[6], _t("smedit6", L"Link source buttons to plugins"));
	//! Adjust sourceflow coverflow
	m_btnMgr.setText(m_configLbl[7], _t("cfgc4", L"Adjust coverflow"));
	//! Delete sourceflow cache files
	m_btnMgr.setText(m_configLbl[8], _t("cfgsm5", L"Clear sourceflow cache"));
	m_btnMgr.setText(m_configBtn[8], _t("cfgbnr6", L"Delete"));
	m_btnMgr.show(m_configBtn[8], instant);
...
void CMenu::_configSource(void)
{
	...
	while(!m_exit)
	{
		...
		else if(BTN_A_OR_2_PRESSED)
		{
			...
			if(m_btnMgr.selected(m_checkboxBtn[0])) // SM on start
			{
				m_cfg.setBool(general_domain, "source_on_start", !m_cfg.getBool(general_domain, "source_on_start"));
				_showConfigSource(true);
				m_btnMgr.setSelected(m_checkboxBtn[0]);
			}
			else if(m_btnMgr.selected(m_checkboxBtn[1])) // enable SF
			{
			...
			else if(m_btnMgr.selected(m_checkboxBtn[2])) // smallbox
			{
			...
			else if(m_btnMgr.selected(m_checkboxBtn[4])) // sourceflow alpha
			{
			...
			else if(m_btnMgr.selected(m_configBtnGo[5])) // hide source buttons
			{
				_hideConfig(true);
				_srcTierBack(true);
			...
			else if(m_btnMgr.selected(m_configBtnGo[6])) // link source buttons to plugins
			{
				_hideConfig(true);
				_srcTierBack(true);
			...
			else if(m_btnMgr.selected(m_configBtnGo[7])) // adjust CF
			{
				_hideConfig();
				m_sourceflow = true;
				_showCF(true);
	...
	if((prev_SF_enabled != SF_enabled) || (cur_smallbox != prev_smallbox) || (cur_box_mode != prev_box_mode) || clear_SF)
	{
