...
{
	snprintf(filepath, ISFS_MAXPATH, "/title/%08lx/%08lx/content/%08lx.app", TITLE_UPPER(title), TITLE_LOWER(title), (unsigned long)bootcontent);
	...
	*IOS = data[0x18B];
	...
	if(dol)
	{
        ...
        for(u32 i = 0; i < tmd_file->num_contents; ++i)
        {
			...
			snprintf(filepath, ISFS_MAXPATH, "/title/%08lx/%08lx/content/%08lx.app", TITLE_UPPER(title), TITLE_LOWER(title), (unsigned long)tmd_file->contents[i].cid);
...
static u32 MoveDol(u8 *buffer)
{
	...
	for(u8 i = 0; i < 18; i++)
	{
		...
		if(dolfile->section_pos[i] < sizeof(dolheader)) // if section position is within the header don't move
			continue;
		if(!(dolfile->section_start[i] & 0x80000000)) // make sure section start is 80000000 or greater
